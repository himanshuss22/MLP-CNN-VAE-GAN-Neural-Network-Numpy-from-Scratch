...
class ConvLayer2D(object):
    ...
    def get_output_size(self, input_size):
        ...
        output_shape = (batch_size, output_height, output_width, self.number_filters)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        return output_shape
    ...
    def forward(self, img):
        ...
        for h in range(output_height):
          for w in range(output_width):
            ...
            output[:,h,w,:] = np.sum(a_slice_prev * weights, axis=(1,2,3))
    ...
    def forward(self, img):
        ...
        for h in range(output_height):
            for w in range(output_width):
                ...
                img_pool = img[:, vert_start:vert_end, horiz_start:horiz_end, :]
                output[:, h, w, :] = np.amax(img_pool, axis=(1, 2))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = img
