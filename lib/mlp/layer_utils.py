...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        #flatten: (batch, a, b, c, ...) into (batch, a*b*c*...)
...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        # output = wx + b 
        # dloss/doutput = dprev 
        self.grads[self.w_name] = feat.T @ dprev # grad(w): dloss/dw = x.T * dprev, (12, 15) * (15, 15) = (12, 15)
        self.grads[self.b_name] = np.sum(dprev, axis=0) # grad(b): (15, )
        dfeat = dprev @ self.params[self.w_name].T # grad(x): dloss/dx = dprev * w.T, (15, 15) * (15, 12) = (15, 12)
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        tanh_term = np.tanh(np.sqrt(2 / np.pi) * (feat + 0.044715 * np.power(feat, 3)))
        doutput_df = 0.5 * (1 + tanh_term) + \
                0.5 * feat * (1 - tanh_term ** 2) * np.sqrt(2 / np.pi) * (1 + 0.134145 * np.power(feat, 2))
        dfeat = dprev * doutput_df
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob > 0:
            # Dropout mask only if training and keep_prob is greater than 0
            kept = self.rng.binomial(1, self.keep_prob, size=feat.shape).astype(np.float32)
            # Applying the dropout mask and scale the features
            output = feat * kept / self.keep_prob
        else:
            # No dropout; output is the same as input features
            output = feat
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = dprev * self.kept / self.keep_prob if self.is_training and self.keep_prob > 0 else dprev
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.is_training = False
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        loss = -np.mean(np.log(logit[range(label.shape[0]), label] + 1e-20))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
